...
class ConditionalImitations(KerasCategorical):
    """
    A Keras part that take an image and Behavior vector as input,
    outputs steering and throttle
    """
...
class KerasConditionalImitation(KerasCategorical):
    """
    A Keras part that take an image and conditional command as input,
    outputs steering and throttle
    """
    ...
    def __init__(self,
                 ...
                 input_shape: Tuple[int, ...] = (120, 160, 3),
                 throttle_range: float = 0.5,
                 num_conditional_inputs: int = 1):
        self.num_conditional_inputs = num_conditional_inputs
        super().__init__(interpreter, input_shape, throttle_range)

    def create_model(self):

        num_cond_inputs = self.num_conditional_inputs
        input_shape = self.input_shape

        drop = 0.2
        img_in = Input(shape=input_shape, name='img_in')
        # tensorflow is ordering the model inputs alphabetically in tensorrt,
        # so condition must come after image, hence we put an x here in front.
        ccmd_in = Input(shape=(num_cond_inputs,), name="xconditional_in")
        ...
        y = ccmd_in
        y = Dense(num_cond_inputs * 2, activation='relu')(y)
        y = Dense(num_cond_inputs * 2, activation='relu')(y)
        y = Dense(num_cond_inputs * 2, activation='relu')(y)
        ...
        model = Model(inputs=[img_in, ccmd_in], outputs=[angle_out, throttle_out],
                      name='conditional')
        return model
    ...
    def x_transform(self, record: Union[TubRecord, List[TubRecord]]) -> XY:
        assert isinstance(record, TubRecord), 'TubRecord expected'
        img_arr = record.image(cached=True)
        ccmd = record.underlying['user/conditional_command']
        return img_arr, ccmd
    ...
    def x_transform_and_process(
        ...
        # here the image is in first slot of the tuple
        x_img_process = img_processor(x_img)
        return x_img_process, ccmd

    def x_translate(self, x: XY) -> Dict[str, Union[float, np.ndarray]]:
        assert isinstance(x, tuple), 'Tuple required'
        return {'img_in': x[0], 'xconditional_in': x[1]}
    ...
    def output_shapes(self):
        ...
        # the keys need to match the models input/output layers
        shapes = ({'img_in': tf.TensorShape(img_shape),
                   'xconditional_in': tf.TensorShape([self.num_conditional_inputs])},
                  {'angle_out': tf.TensorShape([15]),
                   'throttle_out': tf.TensorShape([20])})
        return shapes
